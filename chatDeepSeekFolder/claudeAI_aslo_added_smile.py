...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS results (
        ...
        savollar_soni INTEGER NOT NULL,  -- Ushbu qator qo‘shildi
        foiz REAL NOT NULL,
...
class EduEvaluationApp:
    ...
    def show_results(self):
        ...
        if not results:
            ...
            for row in results:
                ...
                tree.insert('', END, values=(row[0], row[1], row[2], f"{row[3]:.1f}%", holat, row[5]))
    ...
    def export_results_to_excel(self):
        try:
            # Parametrli query va xavfsizlikni oshirish
            with db_session() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT 
                    ...
                    JOIN users u ON r.oquvchi_id = u.id
                    JOIN tests t ON r.test_id = t.id
                    WHERE t.oqituvchi_id = ? AND r.vaqt IS NOT NULL
                ''', (self.current_user['id'],))
                ...
                if not results:
                    messagebox.showinfo("Ma'lumot", "Hech qanday natija topilmadi.")
                    return
                
                # DataFrame yaratishda qo'shimcha tekshiruvlar
                try:
                    ...
                    # Holat ustunini qayta shakllantirish
                    df["Holat"] = df["Holat"].apply(lambda x: "O'tdi" if x else "O'tmadi")
                    
                    # Fayl saqlash dialogini ochish
                    ...
                    # Faylga yozishdan oldin tekshiruvlar
                    try:
                        # Excel yoki CSV formatida saqlash
                        ...
                        messagebox.showinfo(
                            "Muvaffaqiyat", 
                            f"Natijalar {os.path.basename(file_path)} fayliga saqlandi!"
                        )
                    
                    except PermissionError:
                        messagebox.showerror(
                            "Ruxsat xatosi", 
                            "Faylga yozish uchun ruxsat yo'q. Boshqa joyni tanlang."
                        )
                    except IOError as io_err:
                        messagebox.showerror(
                            "Fayl xatosi", 
                            f"Faylni saqlashda xatolik: {str(io_err)}"
                        )
                
                except pd.errors.DataError as df_err:
                    messagebox.showerror(
                        "Ma'lumot xatosi", 
                        f"Ma'lumotlarni qayta ishlashda xatolik: {str(df_err)}"
                    )
        
        except sqlite3.Error as db_err:
            messagebox.showerror(
                "Database xatosi", 
                f"Ma'lumotlar bazasidan olib bo'lmadi: {str(db_err)}"
            )
        except Exception as e:
            messagebox.showerror(
                "Noma'lum xatolik", 
                f"Kutilmagan xatolik yuz berdi: {str(e)}"
            )
